...
from lxml.html import XHTML_NAMESPACE, _transform_result, defs
...
_looks_like_tag_content = re.compile(
    r"</?[a-zA-Z]+|\son[a-zA-Z]+\s*=", (re.ASCII)
).search
...
class Cleaner:
    ...
    def __init__(self, **kw):
        ...
        self.host_whitelist = (
            frozenset(self.host_whitelist) if self.host_whitelist else ()
        )
    ...
    def allow_embedded_url(self, el, url):
        ...
        if (parts.netloc or authority) and parts.netloc != authority:
            warnings.warn(
                f"It's impossible to parse the hostname from URL: '{url}'! "
