...
class CronTab:
    ...
    def read(self, filename=None):
        ...
        if self.intab is not None:
            ...
            try:
                ...
                raise IOError(
                    f"Can't read crontab; No crontab program '{self.cron_command}'"
                )
    ...
    def append(self, item, line="", read=False, before=None):
        ...
        try:
            ...
            raise ValueError(
                "Can not find CronItem in crontab to insert before"
            ) from err
    ...
    def write(self, filename=None, user=None, errors=False):
        ...
        if not self.filen:
            ...
            try:
                ...
                raise IOError(
                    f"Can't write crontab, no crontab program '{self.cron_command}'"
                )
            ret = proc.wait()
            if ret != 0:
                msg = proc.stderr.read()
                raise IOError(
                    f"Program Error: {self.cron_command} returned {ret}: {msg}"
                )
            proc.stdout.close()
            proc.stderr.close()
            os.unlink(path)
    ...
    def new(
        self, command="", comment="", user=None, pre_comment=False, before=None
...
class CronItem:
    ...
    def schedule(self, date_from=None):
        ...
        try:
            ...
            from croniter.croniter import (  # pylint: disable=import-outside-toplevel
                croniter,
            )
    ...
    def description(self, **kw):
        ...
        **kw - Keyword arguments to pass to cron_descriptor (see docs)
        """
        try:
            from cron_descriptor import (  # pylint: disable=import-outside-toplevel
