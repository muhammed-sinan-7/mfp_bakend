...
class Channel(virtual.Channel):
    ...
    def _create_subscription(
        ...
        subscription_path = subscription_path or self.subscriber.subscription_path(
    ...
    def _get_routing_key(message):
        routing_key = (
            message["properties"].get("delivery_info", {}).get("routing_key", "")
        )
        return routing_key
...
class Transport(virtual.Transport):
    ...
    def _rm_empty_bulk_requests(self):
        empty = {
            f for f in self._get_bulk_future_to_queue if f.done() and f.exception()
        }
        for f in empty:
            self._get_bulk_future_to_queue.pop(f, None)
