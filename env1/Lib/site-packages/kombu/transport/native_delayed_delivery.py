...
def declare_native_delayed_delivery_exchanges_and_queues(
    ...
    for level in range(27, -1, -1):
        ...
        queue_arguments = {
            ...
            "x-dead-letter-exchange": (
                next_level if level > 0 else CELERY_DELAYED_DELIVERY_EXCHANGE
            ),
        }
        ...
        delayed_queue: Queue = Queue(
            current_level, queue_arguments=queue_arguments
        ).bind(channel)
    ...
    delivery_exchange: Exchange = Exchange(
        CELERY_DELAYED_DELIVERY_EXCHANGE, type="topic"
    ).bind(channel)
...
def bind_queue_to_native_delayed_delivery_exchange(
    ...
    for binding_entry in bindings:
        ...
        routing_key = (
            binding_entry.routing_key
            if binding_entry.routing_key.startswith("#")
