...
class AsyncSQSConnection(AsyncAWSQueryConnection):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.message_attribute_names = (
            ...
            if isinstance(message_attribute_names, str)
    ...
    def _create_query_request(self, operation, params, queue_url, method):
        ...
        return AWSRequest(
            method=method, url=queue_url, headers=headers, **param_payload
        )
    ...
    def _create_json_request(self, operation, params, queue_url):
        ...
        param_payload = {"data": json.dumps(params).encode(), "headers": headers}
    ...
    def make_request(
        ...
        if protocol == "query":
            ...
            request = self._create_json_request(operation_name, all_params, queue_url)
        else:
    ...
    def receive_message(
        ...
        attrs = (
            ...
            else self.message_system_attribute_names
        )
        msg_attr_names = (
            self.message_attribute_names if self.message_attribute_names else None
        )
        ...
        if attrs:
            ...
            proto_params["query"].update(
                _query_object_encode({"MessageSystemAttributeName": list(attrs)})
            )
        if msg_attr_names:
            proto_params["json"].update({"MessageAttributeNames": list(msg_attr_names)})
            proto_params["query"].update(
                _query_object_encode({"MessageAttributeNames": list(msg_attr_names)})
            )
    ...
    def delete_message_batch(self, queue, messages, callback=None):
        p_params = {
            "json": {
                "Entries": [
                    {"Id": m.id, "ReceiptHandle": m.receipt_handle} for m in messages
                ],
            },
