...
class Backend:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.content_type, self.content_encoding, self.encoder = (
    ...
    def chord_error_from_stack(self, callback, exc=None):
        ...
        if isinstance(callback, group):
            return self._handle_group_chord_error(
                group_callback=callback, backend=backend, exc=exc
            )
            ...
            if isinstance(frozen_group, GroupResult):
                ...
                for result in frozen_group.results:
                    try:
                        ...
                        try:
                            backend._call_task_errbacks(
                                fake_request, original_exc, None
                            )
    ...
    def _ensure_not_eager(self):
        if (
            ...
            and not self.app.conf.task_store_eager_result
        ):
            warnings.warn(
...
class BaseKeyValueStoreBackend(Backend):
    ...
    def _add_global_keyprefix(self):
        ...
        """
        global_keyprefix = self.app.conf.get(
            "result_backend_transport_options", {}
        ).get("global_keyprefix", None)
        if global_keyprefix:
        ...
        if val > size:  # pragma: no cover
            ...
            try:
                ...
                chord_error = _create_chord_error_with_cause(
                    message=reason, original_exc=exc
                )
