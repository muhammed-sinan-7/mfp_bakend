...
def apply_target(
    target, args=(), kwargs=None, callback=None, accept_callback=None, getpid=None, **_
):
    kwargs = {} if not kwargs else kwargs
    return base.apply_target(
        target, args, kwargs, callback, accept_callback, pid=getpid(), **_
    )
...
class TaskPool(base.BasePool):
    ...
    def _add_to_pool_map(self, pid, greenlet):
        ...
        greenlet.link(
            functools.partial(
                self._cleanup_after_job_finish, pid=pid, pool_map=self._pool_map
            ),
        )
