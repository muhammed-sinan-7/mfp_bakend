...
def during_soft_shutdown(worker: Worker):
    ...
    install_worker_term_hard_handler(
        worker, sig="SIGINT", callback=on_hard_shutdown, verbose=False
    )
...
def on_cold_shutdown(worker: Worker):
    ...
    """
    safe_say(
        "worker: Hitting Ctrl+C again will terminate all running tasks!", sys.__stdout__
    )

    # Replace the signal handler for SIGINT (Ctrl+C) and SIGQUIT (and possibly SIGTERM)
    install_worker_term_hard_handler(
        worker, sig="SIGINT", callback=during_soft_shutdown
    )
    install_worker_term_hard_handler(
        worker, sig="SIGQUIT", callback=during_soft_shutdown
    )
    if REMAP_SIGTERM == "SIGQUIT":
        install_worker_term_hard_handler(
            worker, sig="SIGTERM", callback=during_soft_shutdown
        )
...
if not is_jython:  # pragma: no cover
    ...
    install_worker_term_handler = install_worker_term_hard_handler = (
        lambda *a, **kw: None
    )
