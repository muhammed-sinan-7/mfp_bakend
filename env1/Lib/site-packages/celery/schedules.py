...
class crontab(BaseSchedule):
    ...
    def _delta_to_next(
        self, last_run_at: datetime, next_hour: int, next_minute: int
    ) -> ffwd:
        """Find next delta.
    ...
    def __reduce__(
        self,
    ) -> tuple[type, tuple[Cronspec, Cronspec, Cronspec, Cronspec, Cronspec], Any]:
        return (
            self.__class__,
            (
                self._orig_minute,
...
def maybe_schedule(
    s: int | float | timedelta | BaseSchedule,
    relative: bool = False,
    app: Celery | None = None,
...
class solar(BaseSchedule):
    ...
    def __init__(
        self, event: str, lat: int | float, lon: int | float, **kwargs: Any
    ) -> None:
