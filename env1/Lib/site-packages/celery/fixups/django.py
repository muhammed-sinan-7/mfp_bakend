...
def fixup(
    app: "Celery", env: str = "DJANGO_SETTINGS_MODULE"
...
class DjangoWorkerFixup:
    ...
    def _is_prefork(self) -> bool:
        if self.worker is None:
            return False
        pool = (
            self.worker.pool_cls
            if isinstance(self.worker.pool_cls, str)
            else self.worker.pool_cls.__module__
        )
    ...
    def _close_database(self) -> None:
        ...
        for conn in connections:
            try:
                conn.close()
                pool_enabled = (
                    self._settings.DATABASES.get(conn.alias, {})
                    .get("OPTIONS", {})
                    .get("pool")
                )
