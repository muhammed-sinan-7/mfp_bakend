...
class FastExportParser(object):
    ...
    def _parse_commit(self):
        ...
        committer_name, committer_email, committer_date = self._parse_user(b"committer")
    ...
    def get_config_settings(repo_working_dir):
        ...
        try:
            output = subproc.check_output(
                "git config --list".split(), cwd=repo_working_dir
            )
        ...
        return dict(
            line.split(b"=", maxsplit=1) for line in output.strip().split(b"\n")
        )
...
class RepoAnalyze(object):
    ...
    def run(args):
        ...
        if os.path.isdir(reportdir):
            if args.force:
                sys.stdout.write(
                    _('Warning: Removing recursively: "%s"') % decode(reportdir)
                )
