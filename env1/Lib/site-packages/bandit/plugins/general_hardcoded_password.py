...
def hardcoded_password_string(context):
    ...
    **Config Options:**
    ...
    if isinstance(node._bandit_parent, ast.Assign):
    ...
    elif isinstance(node._bandit_parent, ast.Subscript) and RE_CANDIDATES.search(
        node.value
    ):
        ...
        if isinstance(comp.left, ast.Name):
            if RE_CANDIDATES.search(comp.left.id):
                if isinstance(comp.comparators[0], ast.Constant) and isinstance(
                    comp.comparators[0].value, str
                ):
