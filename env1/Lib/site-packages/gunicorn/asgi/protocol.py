...
class ASGIProtocol(asyncio.Protocol):
    ...
    def _setup_callback_parser(self):
        ...
        if parser_setting == "python":
            ...
            if (
                self._check_h1c_protocol_available()
                and ASGIProtocol._h1c_has_limits
    ...
    def connection_lost(self, exc):
        ...
        if self._task and not self._task.done():
            ...
            if grace_period > 0:
                self.worker.loop.call_later(grace_period, self._cancel_task_if_pending)
            else:
    ...
    async def _handle_connection(self):
        ...
        try:
            ...
            while not self._closed:
                ...
                # Handle HTTP request
                keepalive = await self._handle_http_request(request, sockname, peername)
        ...
        while not self._closed:
            ...
            try:
                request = await AsyncUWSGIRequest.parse(
                    self.cfg, unreader, peername, self.req_count
                )
            except NoMoreData:
                break
    ...
    async def _handle_websocket(self, request, sockname, peername):
        ...
        ws_protocol = WebSocketProtocol(self.transport, scope, self.app, self.log)
        ...
        async def send(message):
            ...
            if msg_type == "http.response.start":
                ...
                has_content_length = any(
                    (name.lower() if isinstance(name, str) else name.lower())
                    == b"content-length"
                    or (name.lower() if isinstance(name, str) else name.lower())
                    == "content-length"
    ...
    async def _handle_http2_connection(self, transport, ssl_object):
        ...
        try:
            ...
            writer = asyncio.StreamWriter(transport, protocol, reader, self.worker.loop)

            # Create HTTP/2 connection handler
            h2_conn = AsyncHTTP2Connection(self.cfg, reader, writer, peername)
    ...
    async def _handle_http2_request(self, request, h2_conn, sockname, peername):
        ...
        async def receive():
            ...
            if (
                not body_received
                and stream.request_complete
                and not stream._body_chunks
            ):
                body_received = True
            ...
            try:
                chunk = await asyncio.wait_for(stream.read_body_chunk(), timeout=30.0)
            except asyncio.TimeoutError:
                return {"type": "http.disconnect"}
            ...
            if msg_type == "http.response.start":
                ...
                if not response_complete:
                    exc_to_raise = RuntimeError(
                        "Cannot send trailers before body complete"
                    )
                    return
    ...
    def _build_http2_scope(self, request, sockname, peername):
        ...
        scope = {
            ...
            or (request.path.encode("latin-1") if request.path else b""),
            "query_string": request.query.encode("latin-1") if request.query else b"",
            "root_path": self.cfg.root_path or "",
            "headers": headers,
            "server": server,
