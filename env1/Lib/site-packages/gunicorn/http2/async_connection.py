        ...
        try:
            if timeout is not None:
                data = await asyncio.wait_for(
                    self.reader.read(self.READ_BUFFER_SIZE), timeout=timeout
                )
            else:
            ...
            try:
                ...
                try:
                    self.h2_conn.close_connection(
                        error_code=HTTP2ErrorCode.FLOW_CONTROL_ERROR
                    )
                except Exception:
                    pass
    ...
    async def send_response(self, stream_id, status, headers, body=None):
        ...
        try:
            ...
            self.h2_conn.send_headers(
                stream_id, response_headers, end_stream=end_stream
            )
        ...
        for _ in range(max_wait_attempts):
            ...
            try:
                incoming = await asyncio.wait_for(
                    self.reader.read(self.READ_BUFFER_SIZE), timeout=0.1
                )
                if incoming:
