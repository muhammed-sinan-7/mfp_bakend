...
class DirtyArbiter:
    ...
    SIGNALS = [
        ...
        for x in "HUP QUIT INT TERM TTIN TTOU USR1 USR2 CHLD".split()
    ]
    ...
    def __init__(self, cfg, log, socket_path=None, pidfile=None):
        ...
        self.socket_path = socket_path or os.path.join(self.tmpdir, "arbiter.sock")
        ...
        for spec in self.cfg.dirty_apps:
            ...
            if worker_count is None:
                try:
                    ...
                    self.log.warning(
                        "Could not read workers attribute from %s: %s", import_path, e
                    )
    ...
    def _signal_handler(self, sig, frame):
        ...
        if sig == signal.SIGTTIN:
            ...
            self.num_workers += 1
            self.log.info("SIGTTIN: Increasing dirty workers to %s", self.num_workers)
            if self._loop:
                self._loop.call_soon_threadsafe(
        ...
        if sig == signal.SIGTTOU:
            ...
            if self.num_workers <= min_workers:
                ...
                return
            self.num_workers -= 1
            self.log.info("SIGTTOU: Decreasing dirty workers to %s", self.num_workers)
            if self._loop:
                self._loop.call_soon_threadsafe(
                    lambda: asyncio.create_task(self.manage_workers())
                )
    ...
    async def _execute_on_worker(self, worker_pid, request, client_writer):
        ...
        try:
            ...
            response = make_error_response(
                request_id,
                DirtyWorkerError(
                    f"Worker communication failed: {e}", worker_id=worker_pid
                ),
            )
            await DirtyProtocol.write_message_async(client_writer, response)
        ...
        try:
            if op == MANAGE_OP_ADD:
                ...
                for _ in range(count):
                    ...
                    # Kill oldest worker
                    oldest_pid = min(
                        self.workers.keys(), key=lambda p: self.workers[p].age
                    )
            ...
            self.log.info(
                "Worker management: %s %d workers (spawned/removed: %d)",
        ...
        try:
            ...
            if op == STASH_OP_PUT:
                ...
                if table not in self.stash_tables:
                    ...
                    if pattern:
                        all_keys = [
                            k for k in all_keys if fnmatch.fnmatch(str(k), pattern)
                        ]
                    result = all_keys
    ...
    async def manage_workers(self):
        ...
        while len(self.workers) > num_workers:
            # Kill oldest worker
            oldest_pid = min(self.workers.keys(), key=lambda p: self.workers[p].age)
            self.kill_worker(oldest_pid, signal.SIGTERM)
            await asyncio.sleep(0.1)
    ...
    def spawn_worker(self, force_all_apps=False):
        ...
        self.worker_age += 1
        socket_path = os.path.join(self.tmpdir, f"worker-{self.worker_age}.sock")
        ...
        if pid != 0:
            ...
            self.log.info(
                "Spawned dirty worker (pid: %s) with apps: %s", pid, app_paths
            )
            return pid
    ...
    async def murder_workers(self):
        ...
        for pid, worker in list(self.workers.items()):
            try:
                if (
                    time.monotonic() - worker.tmp.last_update()
                    <= self.cfg.dirty_timeout
                ):
                    continue
        ...
        try:
            while True:
                ...
                if os.WIFEXITED(status):
                    ...
                    self.log.warning(
                        "Dirty worker (pid:%s) killed by signal %s", wpid, sig
                    )
