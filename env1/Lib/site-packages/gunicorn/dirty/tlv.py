...
class TLVEncoder:
    ...
    def encode(value) -> bytes:  # pylint: disable=too-many-return-statements
        ...
        if isinstance(value, bytes):
            if len(value) > MAX_BYTES_SIZE:
                raise DirtyProtocolError(
                    f"Bytes too large: {len(value)} bytes " f"(max: {MAX_BYTES_SIZE})"
                )
        ...
        if isinstance(value, (list, tuple)):
            if len(value) > MAX_LIST_SIZE:
                raise DirtyProtocolError(
                    f"List too large: {len(value)} items " f"(max: {MAX_LIST_SIZE})"
                )
        ...
        if isinstance(value, dict):
            if len(value) > MAX_DICT_SIZE:
                raise DirtyProtocolError(
                    f"Dict too large: {len(value)} items " f"(max: {MAX_DICT_SIZE})"
                )
    ...
    def decode(
    ...
    ) -> tuple:  # pylint: disable=too-many-return-statements
        """
        ...
        if offset >= len(data):
            raise DirtyProtocolError(
                "Truncated TLV data: no type byte", raw_data=data[offset : offset + 20]
            )
