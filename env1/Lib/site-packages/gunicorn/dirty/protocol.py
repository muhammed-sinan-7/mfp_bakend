...
class BinaryProtocol:
    ...
    def encode_header(msg_type: int, request_id: int, payload_length: int) -> bytes:
        ...
        """
        return struct.pack(
            HEADER_FORMAT, MAGIC, VERSION, msg_type, payload_length, request_id
        )
    ...
    def decode_header(data: bytes) -> tuple:
        ...
        if magic != MAGIC:
            raise DirtyProtocolError(
                f"Invalid magic: {magic!r}, expected {MAGIC!r}", raw_data=data[:20]
            )
        ...
        if msg_type not in MSG_TYPE_TO_STR:
            raise DirtyProtocolError(
                f"Unknown message type: 0x{msg_type:02x}", raw_data=data[:20]
            )
    ...
    def encode_request(
        ...
        header = BinaryProtocol.encode_header(
            MSG_TYPE_REQUEST, request_id, len(payload)
        )
    ...
    def encode_response(request_id: int, result) -> bytes:
        ...
        payload = TLVEncoder.encode(payload_dict)
        header = BinaryProtocol.encode_header(
            MSG_TYPE_RESPONSE, request_id, len(payload)
        )
        return header + payload
    ...
    def encode_error(request_id: int, error) -> bytes:
        ...
        payload = TLVEncoder.encode(payload_dict)
        header = BinaryProtocol.encode_header(MSG_TYPE_ERROR, request_id, len(payload))
        return header + payload
    ...
    def encode_chunk(request_id: int, data) -> bytes:
        ...
        payload = TLVEncoder.encode(payload_dict)
        header = BinaryProtocol.encode_header(MSG_TYPE_CHUNK, request_id, len(payload))
        return header + payload
    ...
    def encode_manage(request_id: int, op: int, count: int = 1) -> bytes:
        ...
        payload = TLVEncoder.encode(payload_dict)
        header = BinaryProtocol.encode_header(MSG_TYPE_MANAGE, request_id, len(payload))
        return header + payload

    @staticmethod
    def encode_stash(
        request_id: int, op: int, table: str, key=None, value=None, pattern=None
    ) -> bytes:
        """
        ...
        payload = TLVEncoder.encode(payload_dict)
        header = BinaryProtocol.encode_header(MSG_TYPE_STASH, request_id, len(payload))
        return header + payload
    ...
    def decode_message(data: bytes) -> tuple:
        ...
        if length == 0:
            ...
            try:
                ...
                raise DirtyProtocolError(
                    f"Failed to decode TLV payload: {e}", raw_data=payload_data[:50]
                )
    ...
    @staticmethod
    async def write_message_async(writer: asyncio.StreamWriter, message: dict) -> None:
        """
    ...
    def _encode_from_dict(
        ...
        if msg_type == MSG_TYPE_REQUEST:
            ...
            return BinaryProtocol.encode_response(request_id, message.get("result"))
        elif msg_type == MSG_TYPE_ERROR:
            return BinaryProtocol.encode_error(request_id, message.get("error", {}))
        elif msg_type == MSG_TYPE_CHUNK:
            return BinaryProtocol.encode_chunk(request_id, message.get("data"))
            ...
            return BinaryProtocol.encode_manage(
                request_id, message.get("op"), message.get("count", 1)
            )
        else:
...
def make_request(
    request_id, app_path: str, action: str, args: tuple = None, kwargs: dict = None
) -> dict:
    """
...
def make_stash_message(
    request_id, op: int, table: str, key=None, value=None, pattern=None
) -> dict:
    """
